...
# split Solexa fastq into batches, each with 4 million lines 
...
# maxMismatch, maxDist
  ...
  if optHash.key?("--batchSize")
    batchSize = optHash["--batchSize"].to_i 
  else
    batchSize = 1000000  # default 500,000 reads per batch
  end
...
# get the file names
  files = Dir.entries(input).sort.select {|file| file.match(".fastq$") }
...
#    cmd = %q[awk '{print "@"$1":"$2":"$3":"$4":"$5"/"$6"\n"$7"\n+\n"$8}'] +  " #{absFile} " + %q[ | split -l ] + " #{batchSize} - #{absOutPrefix}" 
    cmd = "#{$maq} fastq2bfq -n #{batchSize} #{absFile} #{absOutPrefix} 2> /dev/null"
    system(cmd)
...
def pairedEndMapping(dir, ref, flag, log, maxMismatch, maxDist)
  ...
  bfqsFreads.each do |fReads|
    ...
    if File.exist?(r2)
      ...
      maqstring = "#{$maq} map  -n #{maxMismatch} -a #{maxDist}  #{mapresult}  #{ref} #{r1} 2>> #{log}"
