...
puts "order\tswap\tbaseChangeType\tHomoPolymerSize\tEnv\tQual\tDist3\tDist5\tTail\tSub\tIndel\tScoreRatio\tReadSize"
...
#  $stderr.puts "#{refbase}\t#{snpbase}"
  ...
  info.split(';').each do |r|
    if r =~ /^(\S)\((\d+)\)(\S+)\((\d+)\)\((\S+)\/(\d+)\)([+|-])(\S+)\((\S+)\/(\S+)\/(\d+)\)(\S+)/
      qual,name,dist3,score,size, dir, renv,subs,indels,tail, kind = $2.to_i, $3, $4.to_i, $5.to_f, $6.to_i, $7, $8, $9.to_f,$10.to_f, $11.to_i, $12
...
#      snp = {:baseChange => trans, :homoSize => homo, :env => env, :qual => qual, :readName => name,  :dist => dist, :readScore => score,  :readSize => size, :subs => subs, :indels => indels, :tail =>tail, :sawp =>  swap }
      ...
      #       puts "#{num}\t#{trans}\t#{homol}\t#{homoc}\t#{homor}\t#{env}\t#{qual}\t#{dist3}\t#{dist5}\t#{score/size}\t#{size}"
      puts "#{num}\t#{swap}\t#{trans}\t#{homo}\t#{env}\t#{qual}\t#{dist3}\t#{dist5}\t#{tail}\t#{subs}\t#{indels}\t#{score/size}\t#{size}"
